import { defineStructure, type StructureSpec, type InferStructure } from '@varlabs/ai/utils/structure';

export type SignatureSpec<Input extends StructureSpec, Output extends StructureSpec> = {
    input: Input;
    output: Output;
    instructions?: string;
};

export type Signature<Input extends StructureSpec, Output extends StructureSpec> = {
...
export const defineSignature = <Input extends StructureSpec, Output extends StructureSpec>(
    spec: SignatureSpec<Input, Output>
    ...
    const compilePrompt = (input: InferStructure<Input>): string => {
        ...
        for (const key of Object.keys(spec.input)) {
            lines.push(`${key}: ${JSON.stringify((input as Record<string, unknown>)[key])}`);
        }
