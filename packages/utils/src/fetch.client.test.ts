...
describe('fetch.client', () => {
    ...
    it('throws the parsed error body on a 4xx/5xx status', async () => {
        (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(mockResponse({ message: 'bad request' }, 400));
    ...
    it('aborts and throws Timeout when MAX_FETCH_TIME elapses', async () => {
        (global.fetch as ReturnType<typeof vi.fn>).mockImplementationOnce((_url: string, options: { signal: AbortSignal }) => {
