...
export const useFileUpload = (fileStorage: FileStorage) => {
    ...
    const addFile = useCallback(async (file: { name: string; type: string; size: number }, data: Uint8Array) => {
        setUploading(true);
        try {
            ...
            await fetch(uploadUrl, { method: 'PUT', body: data, headers: { 'Content-Type': file.type } });
            const attachment: Attachment = { fileId, name: file.name, type: file.type, size: file.size };
