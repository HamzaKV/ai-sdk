...
export type Scorer<Output> = (
    output: Output,
    expected: Output,
) => Promise<number>;

export const exactMatchScorer = (options?: {
...
export const llmJudgeScorer = (options: {
    ...
    return async (output, expected) => {
        const response = await options.judge(
            buildJudgePrompt(output, expected, options.rubric),
        );
        const score = Number.parseFloat(response.trim());
        if (Number.isNaN(score)) {
            throw new Error(
                `LLM judge did not return a parseable score: "${response}"`,
            );
        }
